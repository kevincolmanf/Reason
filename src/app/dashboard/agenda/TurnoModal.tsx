...
export default function TurnoModal({ userId, orgId, orgName, professionals, areas, turno, defaultStart, slotInterval, onClose, onSaved, onClone }: Props) {
  const isEdit = !!turno
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-bg-secondary border-[0.5px] border-border rounded-2xl p-6 w-full max-w-[480px] shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        {/* WhatsApp reminder */}
        {!form.is_blocked && form.patient_phone.trim() && form.start_time && form.end_time && (
          <a
            href={buildWhatsAppUrl(form.patient_phone, form.patient_name, form.start_time, form.end_time, form.area, orgName)}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 mt-4 w-full justify-center bg-green-500/10 hover:bg-green-500/20 border-[0.5px] border-green-500/30 rounded-lg px-4 py-2.5 text-[13px] text-green-400 font-medium transition-colors"
          >
