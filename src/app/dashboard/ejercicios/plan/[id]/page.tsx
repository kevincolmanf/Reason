...
export default async function PlanEditorPage({ params }: { params: { id: string } }) {
  ...
  if (plan.patient_id) {
    const [{ data: pt }, { data: evs }, { data: rts }, { data: dyn }] = await Promise.all([
  ...
  return (
    <div className="min-h-screen bg-bg-primary flex flex-col">
      ...
      <main className="flex-grow w-full max-w-[1440px] mx-auto px-4 sm:px-8 py-8">
        ...
        <PlanEditor initialPlan={plan} userId={user.id} initialEvents={events} rtsEvals={rtsEvals} dynEvals={dynEvals} />
      </main>
    </div>
  )
}
