...
export default function EventosClient({ userId, orgId, initialEvents, initialCounts }: Props) {
  ...
  const [form, setForm] = useState({
    title: '', description: '', location: '', cover_emoji: '🎟️',
    date: today, startTime: '19:00', endTime: '', capacity: '',
  })
  ...
  const createEvent = async () => {
    ...
    const ends_at = form.endTime ? new Date(`${form.date}T${form.endTime}`).toISOString() : null
    ...
    setForm({ title: '', description: '', location: '', cover_emoji: '🎟️', date: today, startTime: '19:00', endTime: '', capacity: '' })
    setShowForm(false)
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        {!showForm && (
          <button onClick={() => setShowForm(true)} className="bg-accent text-bg-primary px-4 py-2 rounded-lg text-[13px] font-medium hover:opacity-90 transition-opacity">+ Nuevo evento</button>
        )}
      </div>

      {showForm && (
        <div className="bg-bg-secondary rounded-xl border-[0.5px] border-border p-5 mb-6">
          <div className="text-[13px] font-medium mb-4">Nuevo evento</div>
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={createEvent} disabled={saving} className="bg-accent text-bg-primary px-5 py-2.5 rounded-lg text-[13px] font-medium hover:opacity-90 disabled:opacity-40">{saving ? 'Creando…' : 'Crear evento'}</button>
            {events.length > 0 && <button onClick={() => { setShowForm(false); setError('') }} className="text-[13px] text-text-secondary hover:text-text-primary px-2">Cancelar</button>}
          </div>
        </div>
      )}
          ...
          {events.map(e => (
            <li key={e.id} className="bg-bg-secondary rounded-xl border-[0.5px] border-border overflow-hidden">
              ...
              <div className="flex flex-wrap items-center gap-2 px-4 pb-4">
                <button onClick={() => copyLink(e.public_token)} className="text-[12px] bg-bg-primary border-[0.5px] border-border rounded-lg px-3 py-1.5 hover:border-accent transition-colors">
                  {copied === e.public_token ? '¡Copiado!' : 'Copiar link de inscripción'}
                </button>
