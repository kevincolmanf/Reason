...
// Borra una atención (de cualquier día): alguien puede cargar algo y arrepentirse.
export async function DELETE(request: Request) {
  const supabase = createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'No autorizado' }, { status: 401 })

  let body: { attentionId?: string }
  try { body = await request.json() } catch { return NextResponse.json({ error: 'Body inválido' }, { status: 400 }) }
  const { attentionId } = body
  if (!attentionId) return NextResponse.json({ error: 'Falta attentionId' }, { status: 400 })
  ...
  const { data: att } = await admin
    ...
    .eq('id', attentionId)
    .single()
  if (!att) return NextResponse.json({ error: 'Atención no encontrada' }, { status: 404 })

  const { data: patient } = await admin
    .from('patients')
    .select('user_id, org_id')
    .eq('id', att.patient_id)
    .single()
  if (!patient) return NextResponse.json({ error: 'Paciente no encontrado' }, { status: 404 })
  ...
  const { error } = await admin.from('kine_attentions').delete().eq('id', attentionId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
