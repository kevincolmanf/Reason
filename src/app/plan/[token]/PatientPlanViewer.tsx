...
export default function PatientPlanViewer({ planData, token }: { planData: { sessions: PlanSession[] }, token: string }) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) setReportedExercises(JSON.parse(saved))
    } catch { /* empty */ }
  }, [token])
  ...
  const handleSubmitReport = async (ex: PlanExercise, sessionId: string) => {
    ...
    try {
      const res = await fetch(`/api/paciente/${token}/log`, {
        method: 'POST',
      ...
      if (res.ok) {
        const key = `${ex.id}_${activeWeek}`
        const updated = { ...reportedExercises, [key]: true }
        setReportedExercises(updated)
        localStorage.setItem(`reason_reported_${token}`, JSON.stringify(updated))
                  ...
                  <div key={ex.id} className="flex flex-col">
                    <div className="p-4 sm:p-5 flex flex-col sm:flex-row gap-4 sm:items-center">
                      ...
                      <div className="flex-shrink-0 w-full sm:w-[120px]">
                        ...
                        {ex.youtube_url && (
                          <a href={ex.youtube_url} target="_blank" rel="noreferrer" className="w-full text-center text-[11px] text-accent mt-2 block hover:underline">
                            Ver en YouTube
                          </a>
                        )}
                      </div>
                      ...
                      {!isReporting ? (
                        ...
                        >
                          {isReported ? (
                            ...
                            'Registrar cómo me fue (opcional)'
                          )}
                        </button>
                      ) : (
                        <div className="bg-bg-secondary border-[0.5px] border-border rounded-xl p-4 mt-2">
                          <div className="mb-5">
                          ...
                          <div className="mb-5">
                            ...
                            />
                            <div className="flex justify-between text-[11px] text-text-secondary mt-1">
                              <span>Sin dolor</span><span>Moderado</span><span>Intenso</span>
                            </div>
                          </div>

                          <div className="mb-5">
                            <label className="block text-[13px] font-medium text-text-primary mb-2">Notas (opcional)</label>
                            <textarea
                              value={notes} onChange={e => setNotes(e.target.value)}
                              placeholder="Ej: Me molestó un poco al bajar..."
                              className="w-full bg-bg-primary border-[0.5px] border-border-strong rounded-lg p-3 text-[13px] focus:outline-none focus:border-accent min-h-[72px] resize-none"
                              maxLength={300}
                            />
                          </div>
                          ...
                          <div className="flex gap-3">
                            ...
                            <button
                              ...
                              className="flex-1 py-2 rounded-lg text-[13px] font-medium bg-accent text-bg-primary hover:opacity-90 disabled:opacity-40"
                              disabled={isSubmitting}
                            >
